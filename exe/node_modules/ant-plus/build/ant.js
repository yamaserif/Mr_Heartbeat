    ...
    USBDriver.prototype.open = function () {
        ...
        this.inEp.on('data', function (data) {
            try {
                ...
                var syncIndex = data.indexOf(0xA4);
                if (syncIndex === -1) {
                    return;
                }
                if (syncIndex > 0) {
                    data = data.slice(syncIndex);
                }
                var len = data.length;
                var beginBlock = 0;
                ...
                // Ignore parsing errors to prevent polling loop from dying
            }
        });
        this.inEp.on('error', function (err) {
            //console.log('ERROR RECV: ', err);
            try {
                _this.inEp.startPoll();
            } catch (e) {}
        });
